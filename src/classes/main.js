export class El{
  ...
  Button({path, cName, id, attr, title, text, label, container, style, onclick, disabled, rtn}){
  ...
  Select({path, id, label, lName, func, onchange, onfocus, rtn}){
    if(label) this.l=this.Label({
      path: path,
      cName: lName,
      text: label,
  ...
  Form({path, cName, id, name, action, method, style, func, rtn}){
    ...
    if(id) main.id=id;
    main.name=name;
    if(action) main.action=action;
    method ? main.method=method : main.method='dialog';
    path.appendChild(main);
